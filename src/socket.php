...
class Socket implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        //         'from'  =>  'Servidor',
        //         'msg'   =>  "O usuário {$conn->resourceId} está online!"
        //     ]));
        // }
    }

    /**
     * Recebe a mensagem informada
     *
    ...
    public function onMessage(ConnectionInterface $from, $msg)
    {
        if ($msg == "/help") { # Listando os comandos para o usuário
            $from->send(json_encode([
                'from'  => "Servidor", 
                'msg'   => (
                    "Segue lista de comandos<br><br>/help = Lista os comandos<br>/online = Total de usuários<br>/users = Lista das IDs dos usuários<br>[ID user] + MSG = Enviar mensagem privada"
                )
            ]));
        } else if ($msg == "/online") { # Caso o comando for enviado, o server responde a quantidade apenas para o $from
            $from->send(json_encode([
                'from'  => "Servidor", 
                'msg'   => count($this->clients) . " usuários online!"
            ]));
        } else if ($msg == "/users") { # Caso o comando for enviado para listar os usuários online
            ...
            $privateID = false;
            if (preg_match("/\[([0-9]\d*)\]/",$msg,$arrResult)) {
                $privateID = $arrResult[1]; # Caso seja uma mensagem privada
            }
                ...
                foreach ($this->clients as $client) {
                    if ($UsrIsOnline) {
                        if ($from->resourceId == $client->resourceId) {
                            continue; # Não precisa enviar ao usuário que mandou a msg
                        }

                        if ($privateID == $client->resourceId) {
                            $this->setLog("A Conexão {$client->resourceId} recebeu a mensagem de {$from->resourceId}");
                            $client->send(json_encode([
                                'from'  => "Usuário {$from->resourceId}",
                                'msg'   => "(msg privada)<br>" . trim(str_replace($arrResult[0], "", $msg))
                            ]));
                        }
                    } else {
                        if ($from->resourceId != $client->resourceId) {
                            continue; # Não precisa enviar aos demais usuários
                        }
                        
                        if ($from->resourceId == $client->resourceId) {
                            $client->send(json_encode([
                                'from'  =>  'Servidor',
                                'msg'   =>  "Que pena, o usuário {$privateID} não está conectado!"
                            ]));
                        }
                    }
                }
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        //     $client->send(json_encode([
        //         'from'  =>  'Servidor',
        //         'msg'   =>  "O usuário {$conn->resourceId} saiu!"
        //     ]));
        // }
    }
